...
- **`cli/`** — Clap-based command dispatch. One file per command (init, create, package, worktree, sync, status, exec, config_cmd, prompt_check, pr, doctor). Commands call into `core/`.
- **`core/`** — Business logic. `workspace.rs` parses `meldr.toml`. `config.rs` resolves settings across 5 layers (CLI > env > workspace > global > defaults). `worktree.rs` is the largest file (~2100 lines) — handles worktree creation/removal, sync with conflict detection, parallel fetch, and tmux setup. `state.rs` manages `.meldr/state.json`. `sync_history.rs` handles snapshots for undo. `claude_hooks/` implements the Claude Code hook surface (`session-start`, `stop`, `notify`, `clear`, `selftest`) — the pane resolver (process tree, validated `TMUX_PANE`, server-stamped sidecar, working directory), pane-truth/window-aggregate flashing with deadline-based expiry, and stop-status classification. It never reads `MELDR_TMUX_*`; everything resolves against one `tmux list-panes` snapshot (`tmux/snapshot.rs`) plus process ancestry (`core/proc_table.rs`).
...
- **`tmux/`** — `TmuxOps` trait abstracts tmux. Three preset layouts (default 9-pane: 3 agent panes top + editor + 5 terminals bottom; minimal 2-pane; editor-only) plus custom layouts via template variables. See README "Tmux Integration" for diagrams.
...
- **11 built-in agents** — claude, cursor, gemini, codex, opencode, pi, kiro, kiro-tui, deepseek-tui, devin, antigravity (configurable via global config)
