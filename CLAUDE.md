...
- **`core/`** — Business logic. `workspace.rs` parses `meldr.toml`. `config.rs` resolves settings across 5 layers (CLI > env > workspace > global > defaults). `worktree.rs` is the largest file (~2100 lines) — handles worktree creation/removal, sync with conflict detection, parallel fetch, and tmux setup. `state.rs` manages `.meldr/state.json`. `sync_history.rs` handles snapshots for undo. `claude_hooks/` implements the full Claude Code hook surface (`session-start`, `stop`, `notify`, `register-launcher`) — pane resolver, launcher registry, sidecar files, stop-status classification.
